...
#ifndef SEGGERPRINT_TYPE
#define SEGGERPRINT_TYPE
...
class SeggerPrintf final : public loggerBase<SeggerPrintf>
{
public:
  SeggerPrintf(log_level_e l = LOG_LEVEL_LIMIT()) noexcept
    : loggerBase(l)
  {
    SEGGER_RTT_Init();
  }
...
private:
  friend class loggerBase<SeggerPrintf>;
  template<typename first, typename... Args>
  void _print(first f, Args&&... args)
  {
    static DebugOutputStream sg;
    static TextOutputStream sgop(sg);
    if (level() == log_level_e::logbinary) {
      SEGGER_RTT_Write(0, &f, sizeof(f));
      (

        [&] { SEGGER_RTT_Write(0, &args, sizeof(args)); }(), ...);
    } else {
